...
class OSDUtils: NSObject {
  ...
  /// Whether the custom Tahoe OSD should be used (macOS 26+ and user hasn't disabled it).
  static var useTahoeOSD: Bool {
    isTahoe() && prefs.bool(forKey: PrefKey.enableTahoeOSD.rawValue)
  }
  ...
  static func showOsd(displayID: CGDirectDisplayID, command: Command, value: Float, maxValue: Float = 1, roundChiclet: Bool = false, lock: Bool = false) {
    if useTahoeOSD, #available(macOS 11.0, *) {
